...
// POST /api/experiments — run every dataset item through the model (+ optional judge)
export async function POST(req: NextRequest) {
  ...
  for (const item of items) {
    ...
    // Log every run as a trace (unified observability).
    const trace: Trace = {
      id: newId(),
      createdAt: nowIso(),
      type: "experiment",
    ...
    // Optional LLM-as-judge per item.
...
${output}`;
        ...
        /* judge failure is non-fatal for the item */
      }
    }
  ...
  const avgJudgeScore =
    judgeScores.length > 0
      ? Number((judgeScores.reduce((s, v) => s + v, 0) / judgeScores.length).toFixed(2))
      : null;
